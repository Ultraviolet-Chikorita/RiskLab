"""Core unit and integration tests for RiskLab."""

from risklab.config import APIConfig, LabConfig, ModelProviderType
...
from risklab.risk.thresholds import DecisionOutcome, RiskThresholdManager
from risklab.risk.weights import DEFAULT_WEIGHTS
from risklab.scenarios import EpisodeBuilder, load_default_scenarios
from risklab.scenarios.context import ContextBuilder, ContextMetadata, Domain, StakesLevel
...
class TestScenarios:
    ...
    def test_episode_builder_uses_defaults_when_framings_omitted(self):
        episode = EpisodeBuilder("Test Episode", "Test prompt").build()
        assert len(episode.framings) == 3
...
class TestRiskConditioning:
    ...
    def test_threshold_decisions(self):
        ...
        assert manager.threshold_set.get_outcome(0.5) == DecisionOutcome.MONITOR
        assert manager.threshold_set.get_outcome(0.6) == DecisionOutcome.MITIGATED
...
class TestIntegration:
    ...
    def test_full_heuristic_pipeline(self):
        """Exercise the local evaluation path without external API calls."""
        episode = (
